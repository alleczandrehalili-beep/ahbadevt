...
(function (root) {
  ...
  // Two-button picker (same pattern as the sales form): Camera opens the camera, Album opens the phone's gallery / local files.
  function pickBtns(attr, val, small) {
    var st = small ? ' style="padding:6px 8px;font-size:11px"' : '';
    return '<div class="qa-pick"><label class="qa-btn ghost"' + st + '>📷 Camera<input type="file" accept="image/*" capture="environment" ' + attr + '="' + val + '" hidden></label>' +
           '<label class="qa-btn ghost"' + st + '>🖼 Album<input type="file" accept="image/*" multiple ' + attr + '="' + val + '" hidden></label></div>';
  }
  ...
  function mount(rootEl, o) {
    ...
    function renderForm() {
      ...
      if (!inspected) {
        html += '<div class="qa-sec">Location photo *</div><div class="qa-thumbs" id="qaLocThumbs"></div>' + pickBtns('data-locphoto', '1') +
        ...
        ['OUTSIDE', 'PREMISE'].forEach(function (sec) {
          ...
          checklist().filter(function (c) { return c.section === sec; }).forEach(function (c) {
            ...
            html += '<div class="qa-item" data-item="' + c.id + '"><div class="lbl">' + esc(c.label) + '</div><div class="qa-pfn"><button class="pass ' + (res === 'pass' ? 'on' : '') + '" data-r="pass">PASS</button><button class="fail ' + (res === 'fail' ? 'on' : '') + '" data-r="fail">FAIL</button><button class="na ' + (res === 'na' ? 'on' : '') + '" data-r="na">N/A</button></div>' +
              ...
              (res === 'fail' ? '<div class="qa-fail">' + pickBtns('data-photo', c.id) + '<div class="qa-field" style="margin-top:6px"><label>Violation code *</label><select data-code="' + c.id + '">' + codeOptions(v.code || c.suggested_code) + '</select><div class="qa-hint" data-offhint="' + c.id + '"></div></div><div class="qa-chips" data-chips="' + c.id + '">' + state.cfg.quickRemarks.map(function (q) { return '<span>' + esc(q.label) + '</span>'; }).join('') + '</div><div class="qa-field"><label>Remark</label><textarea rows="2" data-remark="' + c.id + '">' + esc(v.remark || '') + '</textarea></div></div>' :
               '<div style="margin-top:6px">' + pickBtns('data-photo', c.id, true) + '</div>') + '</div>';
          });
        });
      ...
      body.querySelectorAll('[data-photo]').forEach(function (inp) { inp.onchange = function () { Array.prototype.slice.call(inp.files || []).forEach(function (f) { addPhoto(f, Number(inp.dataset.photo)); }); inp.value = ''; }; });
      var locs = body.querySelectorAll('[data-locphoto]'); var loc = locs.length ? locs[0] : null;
      if (loc) { locs.forEach(function (li) { li.onchange = function () { Array.prototype.slice.call(li.files || []).forEach(function (f) { addPhoto(f, null); }); li.value = ''; }; }); var lt = $('#qaLocThumbs'); if (lt) lt.innerHTML = d.photos.concat(d.pending).filter(function (p) { return p.item_id == null; }).map(photoTag).join(''); }
