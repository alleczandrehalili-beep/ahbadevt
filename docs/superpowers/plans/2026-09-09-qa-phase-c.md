...
- Produces (pure, mirrored by SQL in Task 1):
  ...
  - `rectNext(rect, ev)` → `{status, cycle, rectified:boolean}` or `null` (no change). `rect` = `{status, cycle}` or `null` (no loop yet). `ev` = `{type:'submit', visit_status, fails}` | `{type:'assign'}` | `{type:'unassign'}` | `{type:'close'}`.
...
test('rectNext: first fail opens, GOOD re-inspection rectifies, failed re-inspection cycles, NPA never moves the loop', () => {
  ...
  assert.deepEqual(Core.rectNext(open, { type: 'submit', visit_status: 'VISITED', fails: 0 }), { status: 'RECTIFIED', cycle: 1, rectified: true });
  assert.deepEqual(Core.rectNext(open, { type: 'submit', visit_status: 'VISITED', fails: 1 }), { status: 'FOR RECTIFICATION', cycle: 2, rectified: false });
  assert.equal(Core.rectNext(open, { type: 'submit', visit_status: 'UNLOCATED', fails: 0 }), null);
  ...
  assert.equal(Core.rectNext({ status: 'CLOSED', cycle: 2 }, { type: 'submit', visit_status: 'VISITED', fails: 1 }), null);
});
...
```js
    ...
    function afterSubmitRect(a, fails, by) {
      ...
      var nx = Core.rectNext(r ? { status: r.status, cycle: r.cycle } : null, { type: 'submit', visit_status: a.visit_status, fails: fails });
      if (!nx) return;
      if (!r) {
        ...
        Object.assign(r, { status: 'FOR RECTIFICATION', cycle: nx.cycle, last_audit_id: a.id, deadline: plusDays(mnl(iso()), rectDays()), updated_at: iso() }); notify(r.contractor_org_id, r.id, 'opened'); log(a.id, 'rect_cycle', { rect: r.id, cycle: r.cycle, deadline: r.deadline }, by);
      }
      syncJobRect(r);
    }
...
```js
            ...
            (p.violations || []).forEach(function (v) {
              ...
              var amt = v.penalty_amount != null ? Number(v.penalty_amount) : Core.penaltyFor(c, off);
...
```js
      ...
      getRectification: function (id) {
        try { var r = findRect(id); var list = db.audits.filter(function (a) { return a.rectification_id === id && !a.deleted_at; }).sort(function (x, y) { return (x.created_at || '').localeCompare(y.created_at || '') || x.id.localeCompare(y.id); });
...
**Interfaces:**
- Consumes: Task 1 RPC names/argument names exactly: `set_rect_deadline(p_id,p_date,p_reason)`, `assign_reinspection(p_rect_id,p_inspector,p_date,p_seq)`, `close_rectification(p_id,p_reason)`, `override_penalty(p_violation_id,p_amount,p_reason)`, `offense_preview(p_contractor,p_codes)`, `recompute_offenses()`, `monthly_scorecard(p_month)`, `notices_unseen()`, `mark_notices_seen()`.
...
```js
      ...
      getRectification: function (id) {
        var td = new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Manila' });
        return unwrap(qa().from('rectifications').select('*').eq('id', id).single()).then(function (rect) {
          rect.overdue = Core.isOverdue(rect, td);
          return unwrap(qa().from('audits').select('*').eq('rectification_id', id).is('deleted_at', null).order('created_at').order('id')).then(function (audits) {
            var ids = (audits || []).map(function (a) { return a.id; }); if (!ids.length) return { rect: rect, audits: [] };
            return Promise.all([unwrap(qa().from('audit_items').select('*').in('audit_id', ids)), unwrap(qa().from('audit_violations').select('*').in('audit_id', ids).order('id')), unwrap(qa().from('audit_photos').select('*').in('audit_id', ids).order('id'))])
              .then(function (r) { var pick = function (rows, id) { return (rows || []).filter(function (x) { return x.audit_id === id; }); };
                return { rect: rect, audits: audits.map(function (a) { return { audit: a, items: pick(r[0], a.id), violations: pick(r[1], a.id), photos: pick(r[2], a.id) }; }) }; });
          });
        });
      },
    ...
    function openRect(id) {
      ...
      api.getRectification(id).then(function (r) {
        ...
        r.audits.forEach(function (b, i) {
          ...
          html += '<div class="cq-sec">' + (i === 0 ? 'Original inspection' : 'Re-inspection ' + i) + ' · ' + esc(a.id) + ' · ' + fmtWhen(a.inspected_at) + ' · ' + esc(a.inspector || a.assigned_to || '') + ' · ' + esc(a.visit_status || a.status) + (a.assessment ? ' · <b>' + esc(a.assessment) + '</b>' : '') + ' <a href="#" data-aview="' + esc(a.id) + '">open form</a></div>' +
...
- Uses: `qa-sim.js` `create({ org, db })` from Task 3 (shared store → the subcon frame sees the same data).
